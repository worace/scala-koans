...
class AboutLists extends KoanSuite with ShouldMatchers {
  ...
  koan("list indexing doesnt return same objects") {
    val a = List(1, 2, 3)
    val d = List(a(0), a(1), a(2))

    (a eq d) should be(false)
  }
  ...
  koan("Lists can contain multiple types") {
    val a = List(1, "2")
    a(1) should be("2")
    a(0) should be(1)
  }

  koan("Lists with explicit type must only contain that type") {
    //wont compile:
    //val a = List[Int](1, "2")
  }
